...
class Repo:
    ...
    def list_available_year_months(self):
        ...
        cur.execute(
            """
            SELECT DISTINCT TO_CHAR((ts)::timestamp, 'YYYY-MM') ym
            FROM sales_invoices
            ORDER BY ym DESC;
            """
        )
    ...
    def resumen_ventas(self, periodo: str = "mes", product_id: int | None = None,
        ...
        if periodo == "semana":
            ...
            # ordenar por importe total del mes de mayor a menor, luego producto/gramaje
            order = "periodo ASC, importe_gs DESC, producto, gramaje"
...
class App(tk.Tk):
    ...
    def _build_tab_resumen_mes(self, nb):
        ...
        # acciones para expandir/contraer todos los meses
        actions = ttk.Frame(frame)
        actions.pack(fill="x", padx=6, pady=(0, 4))
        ttk.Button(actions, text="Desplegar todo",
                   command=lambda: self._toggle_all_months(expand=True)).pack(side="left", padx=(0, 6))
        ttk.Button(actions, text="Contraer todo",
                   command=lambda: self._toggle_all_months(expand=False)).pack(side="left")
        ...
        # flag para agrupar por mes en nodos expandibles
        self.tv_mes._hier_months = True
    ...
    def _fill_resumen(self, tv: ttk.Treeview, periodo: str):
        ...
        if periodo == "mes":
            if ym:
                kpi_period_label = self._format_month_label(ym)
            elif d1 or d2:
                kpi_period_label = "Rango personalizado"
            else:
                kpi_period_label = "Todos los meses"
        ...
        current_period = None
        current_parent = None
        use_hier = bool(getattr(tv, "_hier_months", False) and periodo == "mes")
        ...
        for r in rows:
            ...
            if show_trend:
                ...
                if period_txt != current_period:
                    ...
                    prev_total = last_month_total
                    if prev_total is None:
                        month_trend = "flat"
                    elif month_total > prev_total:
                        month_trend = "up"
                    elif month_total < prev_total:
                        month_trend = "down"
                    else:
                        month_trend = "flat"
                    pct_month = self._format_trend_pct(prev_total, month_total)
                    importe_label = f"{self._fmt_gs(month_total)} {pct_month}".strip()
                    divider_values = tuple(
                        period_txt if c == "periodo"
                        else importe_label if c == "importe"
                        else "" for c in columns
                    )
                    divider_text = f"Mes: {label}" if label else period_txt
                    divider_tags = ("month_divider", f"trend_{month_trend}")
                    if use_hier:
                        current_parent = tv.insert(
                            "",
                            "end",
                            values=divider_values,
                            text=divider_text,
                            image=trend_icons.get(month_trend),
                            tags=divider_tags,
                            open=False,
                        )
                    else:
            ...
            parent_iid = current_parent if use_hier else ""
            tv.insert(parent_iid, "end", values=row_values, **insert_kwargs)
        ...
        if tv is getattr(self, "tv_mes", None):
            self._update_kpi_mes(total_paq, total_importe, total_base, total_iva, kpi_period_label)
    ...
    def _build_kpi_bar(self, parent):
        """
        Crea una franja de KPIs para la pestaA�a de Mes.
        """
        ...
        self.kpi_vars_mes = {
            ...
            "iva": tk.StringVar(value="0"),
            "periodo": tk.StringVar(value=""),
        }

        def add_card(title, key):
            card = ttk.Frame(cards, padding=(12, 10), style="KPI.TFrame")
            card.pack(side="left", padx=6, fill="x", expand=True)
            ttk.Label(card, text=title, style="KPI.Title.TLabel").pack(anchor="w")
            ttk.Label(card, textvariable=self.kpi_vars_mes[key],
                      style="KPI.Value.TLabel").pack(anchor="w", pady=(2, 0))

        add_card("Paquetes", "paquetes")
        add_card("Importe (Gs)", "importe")
        add_card("Base (Gs)", "base")
        add_card("IVA (Gs)", "iva")

        ttk.Label(bar, textvariable=self.kpi_vars_mes["periodo"],
                  style="KPI.Period.TLabel").pack(side="right", padx=6)

    def _update_kpi_mes(self, total_paq, total_importe, total_base, total_iva, period_label):
        """
        Actualiza los KPIs del tab mensual.
        """
        if not hasattr(self, "kpi_vars_mes"):
            return
        self.kpi_vars_mes["paquetes"].set(f"{int(total_paq):,}".replace(",", "."))
        self.kpi_vars_mes["importe"].set(self._fmt_gs(total_importe))
        self.kpi_vars_mes["base"].set(self._fmt_gs(total_base))
        self.kpi_vars_mes["iva"].set(self._fmt_gs(total_iva))
        self.kpi_vars_mes["periodo"].set(period_label or "")

    def _toggle_all_months(self, expand: bool = True):
        """
        Abre o cierra todos los nodos de mes en el treeview mensual.
        """
        tv = getattr(self, "tv_mes", None)
        if not tv or not getattr(tv, "_hier_months", False):
            return
        for iid in tv.get_children(""):
            tv.item(iid, open=expand)
        self._set_status("Meses desplegados" if expand else "Meses contraidos")
    ...
    def _apply_treeview_striping(self, tree: ttk.Treeview | None):
        ...
        # Si es jerárquico por mes, zebramos dentro de cada mes; si no, lineal
        if getattr(tree, "_hier_months", False):
            for parent in tree.get_children(""):
                ptags = [t for t in tree.item(parent, "tags") if t not in ("evenrow", "oddrow")]
                # no sobreescribimos divisores ni totales
                if "month_divider" not in ptags and "total" not in ptags:
                    ptags.append("evenrow")
                tree.item(parent, tags=tuple(ptags))
                for cidx, child in enumerate(tree.get_children(parent)):
                    ctags = [t for t in tree.item(child, "tags") if t not in ("evenrow", "oddrow")]
                    ctags.append("evenrow" if cidx % 2 == 0 else "oddrow")
                    tree.item(child, tags=tuple(ctags))
        else:
            for idx, iid in enumerate(tree.get_children("")):
                tags = [t for t in tree.item(iid, "tags") if t not in ("evenrow", "oddrow")]
                if "month_divider" not in tags and "total" not in tags:
